...
\begin{abstract}	
This document summarises the findings from an independent security audit commissioned by the Rebel Alliance to investigate the Death Star server. A total of three security vulnerabilities were discovered and exploited successfully along with numerous unexploited vulnerabilities. All three of these exploits can be used to great effect to gain unauthorized access to the target server. One of these granted a remote shell as the root user. Nine out of ten requested pieces of intelligence were recovered. The system's password file was extracted and all user passwords, in one way or another, were cracked.
...
\begin{figure}[H]
	...
	\label{fig:deathstar-cross_section}
\end{figure}
...
\begin{adjustwidth}{1.5cm}{1.5cm}
	...
	\begin{enumerate}
		\item If I was charged with keeping these pieces of information hidden, I would be sure to encrypt the files with a tool such as PGP.
	\end{enumerate}
\end{adjustwidth}
\newpage
