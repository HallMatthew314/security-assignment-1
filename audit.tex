...
\end{verbatim}
%\begin{figure}[H]
%	\includegraphics[width=\linewidth]{resources/sql_injection.png}
...
For one, users tend to reuse the same password for multiple services\footnote{Even though they shouldn't} (see section~\ref{sebsec:bad_passwords}), which means an attacker can find passwords on insecure websites and login on other websites using the same username and password combination.
